...
public class CommuterViz extends PApplet {
...
public void setup() {
  ...
  */
  ...
  for (aState s:states) {
    ...
    s.c=lerpColor(s.c,color(255,255,255),.5f);
    counter++;
  }
...
public void draw() {
  ...
  tempState=states.get(PApplet.parseInt(droplist.getValue()));//tempState is now the currently selected state in dropdown
  ...
  for(int i=0;i<sixValues.length;i++){
    ...
    for(int j=0;j<h.length;j++)
    {
      ...
      h[j]= map(topThreeValues[j],0,PApplet.parseInt(maxValue),0,300);
...
public aState lookupByName(String name)
{
  for(aState s:states)
  {
...
public aState findValue(float value,int catagory)//returns the value from the catagory
{
...
public aState findPercent(float value,int catagory)//returns the value from the catagory
{
...
public float[] topThree(int catagory,float top)//catagory refers to the same system as getSixVallues
{
...
public float[] topThreePercent(int catagory,float top)//catagory refers to the same system as getSixVallues
{
  float[] tempFloat=new float[6];
  float[] compare=new float[51];
  //String[] ret=new String[3];
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "CommuterViz" };
    if (passedArgs != null) {
